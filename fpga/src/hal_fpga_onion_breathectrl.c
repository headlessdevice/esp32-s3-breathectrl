                                        ...
                                        uint32_t breathe_period_ms)
{
    ...
    if(io_pad_num == 22)
    {
        *FPGA_ONION_BREATHECTRL_REG_ADDR_BREATHE_0_CONFIG = (1 << 31) | (clock_cycles_per_step & 0xFFFFFF);
        // i = *FPGA_ONION_BREATHECTRL_REG_ADDR_BREATHE_0_CONFIG;
        // dbg_str_hex32("value written in red LED reg", (uint32_t) clock_cycles_per_step);
    }
    else if(io_pad_num == 21)
    {
        *FPGA_ONION_BREATHECTRL_REG_ADDR_BREATHE_1_CONFIG = (1 << 31) | (clock_cycles_per_step & 0xFFFFFF);
        // i = *FPGA_ONION_BREATHECTRL_REG_ADDR_BREATHE_1_CONFIG;
        // dbg_str_hex32("value written in green LED reg", (uint32_t) clock_cycles_per_step);
    }
    else if(io_pad_num == 18)
    {
        *FPGA_ONION_BREATHECTRL_REG_ADDR_BREATHE_2_CONFIG = (1 << 31) | (clock_cycles_per_step & 0xFFFFFF);
        // i = *FPGA_ONION_BREATHECTRL_REG_ADDR_BREATHE_2_CONFIG;
        // dbg_str_hex32("value written in blue LED reg", (uint32_t) clock_cycles_per_step);
    }
}
